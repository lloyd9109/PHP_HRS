...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        ...
        if (!$roomData) {
            ...
            // Parse the preferred date to calculate the total days
            ...
            if ($checkIn && $checkOut) {
                ...
                // Check if the user has already booked the same room and it is still pending
                $stmt = $pdo->prepare("SELECT * FROM bookings WHERE user_id = ? AND room_number = ? AND status = 'pending'");
                $stmt->execute([$user_id, $roomNumber]);
                $existingBooking = $stmt->fetch(PDO::FETCH_ASSOC);

                if ($existingBooking) {
                    // User has already booked this room and it's pending
                    $_SESSION['confirmationMessage'] = [
                        'type' => 'error',
                        'title' => 'Error Booking',
                        'text' => 'You have already booked this room and it is still pending confirmation. Please wait for the confirmation or choose another room.'
                    ];
                } else {
                    // Check if the room is available for booking (not pending for any other user)
                    $stmt = $pdo->prepare("SELECT * FROM bookings WHERE room_number = ? AND status = 'pending'");
                    $stmt->execute([$roomNumber]);
                    $pendingBooking = $stmt->fetch(PDO::FETCH_ASSOC);

                    if ($pendingBooking) {
                        // Room is pending for another user
                        $_SESSION['confirmationMessage'] = [
                            'type' => 'error',
                            'title' => 'Error Booking',
                            'text' => 'This room is currently pending confirmation for another user. Please try again later or choose another room.'
                        ];
                    } else {
                        // Proceed with the booking insertion
                        $stmt = $pdo->prepare("INSERT INTO bookings (room_number, room_name, full_name, email, phone_number, preferred_date, preferred_time, guests, user_id, price, total_payment, status) 
                                               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending')");
                        $stmt->execute([$roomNumber, $roomName, $fullName, $email, $phone, $preferredDate, $preferredTime, $guests, $user_id, $roomPrice, $totalPayment]);
...
if (isset($_SESSION['confirmationMessage'])) {
    ...
    $borderStyle = '2px solid #FFD700'; // Gold border style
...
<button class="close-btn" onclick="closeBookingModule()">X</button>
    ...
    <div class="title"></div>
        ...
        <div class="form-control">
            <label for="guests2">Guest/s can Accomodate</label>
            <input type="text" id="guests2" name="guests2" readonly>
        </div>
...
function showContainer(roomId) {
    ...
    if (room) {
        ...
        if (room.availability === 'Unavailable' || room.availability === 'Booked'  || room.availability === 'Reserved') {
