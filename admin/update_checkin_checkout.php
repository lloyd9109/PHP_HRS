...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($status == 'complete') {
        $historyQuery = "INSERT INTO booking_history (user_id, room_number, full_name, email, phone_number, preferred_date_start, preferred_date_end, preferred_time, guests, check_in_out, price, total_payment) 
                         VALUES (:user_id, :room_number, :full_name, :email, :phone_number, :preferred_date_start, :preferred_date_end, :preferred_time, :guests, :check_in_out, :price, :total_payment)";
        ...
        $historyStmt->bindParam(':preferred_date_start', $reservation['preferred_date_start']);
        $historyStmt->bindParam(':preferred_date_end', $reservation['preferred_date_end']);
